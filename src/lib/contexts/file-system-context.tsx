...
export function FileSystemProvider({
  ...
  const handleToolCall = useCallback(
    (toolCall: any) => {
      // AI SDK v6 uses `input` (JSON string); v4 used `args` (object)
      const toolName: string = toolCall.toolName;
      const args = toolCall.args ?? (toolCall.input ? JSON.parse(toolCall.input) : null);
